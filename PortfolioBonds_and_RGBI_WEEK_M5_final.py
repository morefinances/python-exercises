...
df = pd.read_csv('C:\\files\\PortfolioRGBI_M5.csv', delimiter=';')
...
plt.text(maxX - 20, nmonthcloseprice[len(closeprice)-1] + 0.015, round(nmonthcloseprice[len(closeprice)-1],2), size=10, color = 'k')

plt.text(maxX - 20, nmonthaver[len(closeprice)-1] - 0.05, round(nmonthaver[len(closeprice)-1],2), size=10, color = '#206497')
...
plt.text(maxX - 80, nmonthcloseprice.max(), textRGBI, size=10, color = 'k')

firstAV = round(aver_line[daystring],2)
lastAV = round(aver_line[len(closeprice)-1],2)
textAV = 'PrtB(0)=' + str(firstAV) + '    PrtB(N)=' + str(lastAV)

plt.text(maxX - 80, nmonthcloseprice.max() - 0.03, textAV, size=10, color = 'k')


filename = currenttime()
plt.savefig('C:\\files\\pctrs\\1W_M5_' + filename + '.png')
plt.savefig('C:\\files\\pctrs\\PortfolioBonds_and_RGBI_1W_M5.png')
