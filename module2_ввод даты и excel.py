...
def excel_read_result():
    ...
    for i in range(len(xl['Участник'])):
        ...
        txt=scrolledtext.ScrolledText(width=51, height=25, wrap=WORD, font=("Courier New", 9))  
        txt.place(x = 40, y = 150)
        if longname == "":
            longname = str(i+1) + ")\t" + "[" + type_format_client(str_xl) + "] " + my_txt(str_xl).rstrip(" ")
        else:
            longname += "\n" +  str(i+1) + ")\t" + "[" + type_format_client(str_xl) + "] " + my_txt(str_xl).rstrip(" ")
