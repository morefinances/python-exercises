...
df = pd.read_csv('C:\\files\\AAA_corpbonds_screener_700D.csv', delimiter=';')
...
plt.plot(yeardatamassive, yearcloseprice, linewidth=3)
...
plt.xticks(np.arange(0, 702 - daystring, (701 - daystring)/10))
...
plt.legend(title='годовая динамика котировок корп.облигаций AAA')
...
if result > 0:
    ...
    plt.text(len(yeardatamassive) - 20, round(yearcloseprice.max(),2) - 1, str(round(result,2)) + "%", size=16, color = 'k', alpha = 0.6, weight='bold')


maxloc = yearcloseprice.idxmax()

plt.text(maxloc + 3 - daystring, round(yearcloseprice.max(),2) - 0.5, round(yearcloseprice.max(),2), size=17, color = '#1F77B4')
...
dh = yearcloseprice.max() - yearcloseprice.min()

dh2 = dh/2
dh2 = dh/2
dh4 = dh/4
stohast25 = yearcloseprice.min() + dh4
stohast75 = yearcloseprice.max() - dh4
stohast10 = yearcloseprice.min() + dh/10
stohast90 = yearcloseprice.max() - dh/10


median = yearcloseprice.min() + dh2
plt.axhline(y=median, color='k', alpha = 0.5, linestyle='-', linewidth=1)
plt.text(len(yeardatamassive)/2, median - 0.1, round(median,2), size=11, color = 'k')


plt.axhline(y=stohast25, color='#1F77B4', linestyle='--', linewidth=1)
plt.text(len(yeardatamassive)/2, stohast25 - 0.16, round(stohast25,2), size=11, color = 'k')



plt.axhline(y=stohast75, color='#1F77B4', linestyle='--', linewidth=1)
plt.text(len(yeardatamassive)/2, stohast75 - 0.15, round(stohast75,2), size=11, color = 'k')

plt.axhline(y=stohast10, color='#1F77B4', linestyle='--', linewidth=1)
plt.text(len(yeardatamassive)/2, stohast10 - 0.15, round(stohast10, 2), size=11, color = 'k')

plt.axhline(y=stohast90, color='#1F77B4', linestyle='--', linewidth=1)
plt.text(len(yeardatamassive)/2, stohast90 - 0.15, round(stohast90,2), size=11, color = 'k')

plt.plot(pointX, pointY, 'o', color = '#1F77B4', ms = 8)
plt.plot(pointX, pointY, 'o', color = 'w', ms = 2)




plt.savefig('C:\\files\\pctrs\\AAA_1Y_' + filename + '.png')
plt.savefig('C:\\files\\pctrs\\AAA_1Y_DAY.png')
