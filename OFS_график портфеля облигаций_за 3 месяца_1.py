...
def findday():
    ...
    if (current_datetime.month - 3) < 10:
        month = '0' + str(current_datetime.month - 3)
    else:
        month = str(current_datetime.month - 3)
...
plt.legend(title='квартальная динамика котировок облигаций, входящих в RGBI')
...
# print(finish_OFS)
plt.text(len(yeardatamassive) - 5, finish_OFS + 0.5, round(finish_OFS,2), size=17, color = '#1F77B4')
#  #color='#11aa55'
...
if result > 0:
    ...
    plt.text(len(yeardatamassive) - 6, 85.6, str(round(result,2)) + "%", size=16, color = 'k', alpha = 0.6, weight='bold')
...
plt.text(minloc - daystring - 6, round(yearcloseprice.min(),2) - 0.2, round(yearcloseprice.min(),2), size=17, color = '#1F77B4')
...
plt.savefig('C:\\files\\pctrs\\3MN_' + filename + '.png')
plt.savefig('C:\\files\\pctrs\\OFS_3MN_DAY.png')
